...
<html lang="pl">
	<head>
		...
		<title>Aktualne - Panel administratorski</title>
	</head>
				...
				<div class="col-sm">
					...
					<table class="table">
						...
						<tbody>
							...
							$statement = $db->prepare("SELECT * FROM depts ORDER BY did ASC");
							$statement->execute();
							$i = 1;
							foreach ($statement->fetchAll(PDO::FETCH_ASSOC) as $row) {
								$depts[] = $row["sid"];
							...
							foreach ($statement->fetchAll(PDO::FETCH_ASSOC) as $row) {
								$name = $row["name"];
								echo '<tr><td>'.$i.'</td><td>'.$name.'</td><td>'.$depts[$i].'</td><td>'.$screen[$depts[$i]].'</td><td>'.$display[$depts[$i]].'</td></tr>';
								$i++;
							}
							?>
						</tbody>
